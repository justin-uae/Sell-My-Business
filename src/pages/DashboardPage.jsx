...
import { getAllProducts, extractOwnerId } from '../services/shopify';
...
const NAV_ITEMS = [
  { icon: 'list_alt', label: 'Listings', id: 'listings' },
  { icon: 'settings', label: 'Settings', id: 'settings' },
  { icon: 'add_business', label: 'List New Business', to: '/sell' },
];
...
export default function DashboardPage() {
    ...
    <aside className="h-full w-64 bg-surface border-r border-outline-variant/30 flex flex-col gap-base p-base shadow-md">
      <div className="px-4 py-6">
        <Link to="/" className="flex items-center gap-2 min-w-0">
          ...
          <Wordmark className="font-bold text-base tracking-tight text-primary truncate min-w-0" />
        </Link>
        ...
        {NAV_ITEMS.map(item => (
          item.to ? (
            <Link
              ...
              className="px-4 py-3 rounded-xl flex items-center gap-3 transition-colors text-left text-on-surface-variant hover:bg-surface-container"
            >
  ...
  return (
    <div className="min-h-screen flex pt-0">
      ...
      {/* Mobile Top Bar */}
      <div className="md:hidden fixed top-0 left-0 right-0 z-40 glass-nav flex items-center justify-between gap-2 px-4 py-3 shadow-sm">
        ...
        <Link to="/" className="flex items-center gap-2 min-w-0 overflow-hidden">
          <Logo className="h-7 w-7" badge />
          <Wordmark className="font-bold text-sm sm:text-lg tracking-tight text-white" />
        </Link>
        <button onClick={handleLogout} className="text-label-sm text-white/70 shrink-0">Sign out</button>
      </div>

      {/* Main Content */}
      <main className="md:ml-64 flex-1 min-w-0 min-h-screen px-margin-mobile md:px-margin-desktop py-10 pt-20 md:pt-20 bg-background">
        {/* Header */}
        <header className="flex flex-col lg:flex-row lg:items-end justify-between gap-6 mb-12">
          ...
          <div className="glass-card px-6 py-4 rounded-2xl flex items-center gap-4 w-full lg:w-auto">
                  ...
                  <div className="md:hidden space-y-3">
                    {myListings.map(listing => (
                      <Link
                        ...
                        className="bg-surface-container-low rounded-xl p-4 flex items-center gap-3 hover:bg-surface-container transition-colors"
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-label-xs text-on-surface-variant truncate mt-0.5">
                            Est. {listing.yearEstablished || '—'} · Revenue {formatCurrency(listing.revenue)}
                          </p>
                    ...
                    <table className="w-full border-collapse">
                      <thead>
                        <tr className="border-b border-outline-variant/30 text-left">
                          {['Business Name', 'Industry', 'Location', 'Year Est.', 'Revenue', 'Status', 'Price'].map(h => (
                            <th key={h} className="pb-4 text-label-xs text-on-surface-variant uppercase tracking-wider font-semibold">
                              {h}
                            </th>
                          ))}
                        </tr>
                      ...
                      <tbody className="divide-y divide-outline-variant/10">
                        {myListings.map(listing => (
                          <tr
                            key={listing.id}
                            onClick={() => navigate(`/listings/${listing.handle}`)}
                          ...
                          >
                            ...
                            <td className="py-5 text-label-sm text-on-surface-variant">{listing.industry || '—'}</td>
                            <td className="py-5 text-label-sm text-on-surface-variant">{listing.location || 'UAE'}</td>
                            <td className="py-5 text-label-sm text-on-surface-variant">{listing.yearEstablished || '—'}</td>
                            <td className="py-5 text-label-sm tabular-nums">{formatCurrency(listing.revenue)}</td>
                            <td className="py-5">
